...
Quest implementazione non gestisce le Case expressions, dunque per gestire gli oggetti vengono utilizzati i costrutti if casePair e caseList, oggetti più generici invece non possono esssere gestiti.
...
list * ::= Nil | Cons * (list *)
~~~
Nil corrisponerà a Pack{1,0} mentre cons corrisponderà a Pack{2,2}, ora per gestire questo tipo di lista implementiamo una nuova premitiva ovvero *caseList* che ha il compito di ritornare un caso base quando ha in input una lista vuota (Nil) altrimenti applicare una funzione ausiliaria sugli elementi della lista. anche *caseList* è stata implementata come una primitiva.
...
Il garbage collector ha il compito di eliminare i nodi che non vengono utilizzati ed è rappresentato dalla funzione *gc* che viene chiamata da *doAdmin* prima di fare uno step se l'heap è troppo grande.
...
Quest implementazione ha reso necessario aggingere il campo Visits al nodo NMarked che ricorda il numero di visite al nodo e che viene messo a Done quando tutti i nodi raggiungibili da esso sono stati marchiati.